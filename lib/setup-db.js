...
export async function setupDatabase() {
  try {
    ...
    for (const collectionName of collections) {
      try {
        ...
        if (error.code === 48) { // Collection already exists
          // console.log(`Collection already exists: ${collectionName}`); // Removed console.log
        } else {
    ...
    // console.log('Created indexes for industry insights collection'); // Removed console.log
...
if (typeof window === 'undefined' && process.argv[1] === new URL(import.meta.url).pathname) {
  setupDatabase()
    .then(() => {
      // console.log('Setup completed'); // Removed console.log
      process.exit(0);
    })
    .catch((error) => {
