...
export const db = {
  ...
  // Coding Stats operations
  codingStats: {
    async findUnique({ where }) {
      const collection = await getCollection(COLLECTIONS.CODING_STATS);
      const query = {};
    ...
    async create({ data }) {
      ...
      const stats = await collection.findOne({ _id: result.insertedId });
      return formatDocument(stats);
    },
